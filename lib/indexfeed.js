...
function parseFacetPath(partial, paginationKey, filter) {
  var facet = false;
  if (partial) {
    var partialKeyStart = partial.indexOf(paginationKey + '_created');
...
function mergeFacet(queryResponse, facetResponse, req, sort, paginationKey) {
  var ee = new events.EventEmitter();
  var queryDone = false;
  var facetDone = false;
  var count = [];
  ...
  facetResponse.on('count', function(row) {
    ...
    if (sort === 'changed') {
      facet.link = req.site.sitePathToUrl(req.sitepath) + '$/' + paginationKey +
        '_changed/' + date.getFullYear() + '_' + (date.getMonth() + 1) + '/';
    } else {
      facet.link = req.site.sitePathToUrl(req.sitepath) + '$/' + paginationKey +
        '_created/' + date.getFullYear() + '_' + (date.getMonth() + 1) + '/';
    }
    count.push(facet);
  });
  queryResponse.on('error', function(err) {
...
function emitDirectBlockWithIdx(req, block, pagination, paginationKey, partial, resp, next) {
  ...
  resp.on('facet', function(f) {
    facet = f;
  });
  resp.on('end', function() {
    req.scheme.renderSync(partial, {site: req.site, data: arts, moreLink: moreLink, facet: facet}, function(err, out) {
      if (err) {
        return next(err);
      }
...
function renderDirectIndexFeed(req, query, view, block, pos, next) {
  ...
  var facet;
  if (block.monthFacet) {
    facet = query.query(db, ctx, req.access, path, block.query,
  ...
  if (facet) {
    resp = mergeFacet(resp, facet, req, block.sort, paginationKey);
  }
