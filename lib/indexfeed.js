...
function resultsToIndexFeed(protoset, scheme, site, response) {
  ...
  response.on('article', function(row) {
    ...
    var article;
    if (protoset) {
      protoset.decorateListing(r, row, scheme, site, function(err, decoratedArticle){
...
function emitDirectBlockWithIdx(site, sitepath, scheme, block, pagination, paginationKey, partial, resp, next) {
  ...
  resp.on('end', function() {
    scheme.renderSync(partial, {site: site, data: arts, moreLink: moreLink, facets: facets}, function(err, out) {
      if (err) {
        return next(err);
      }
 ...
 */
function renderDirectIndexFeed(db, ctx, sitepath, access, protoset, scheme, site, query, block, pos, next) {
  ...
  var idxFeed = addIdx(resultsToIndexFeed(protoset, scheme, site, qr));
