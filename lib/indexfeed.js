...
function installDust(dust, db, cache, query) {
  ...
  dust.helpers.renderDirect = function(chunk, context, bodies, params) {
    return chunk.map(function(chunk) {
      var ctx = context.get('ctx');
      var protoset = context.get('protoset');
      var security = context.get('security');
      ...
      var scheme = context.get('scheme');
      var site = context.get('site');
      var block = {"format":"indexfeed","query":"dir"};
      var pos = context.resolve(params.pos);

      renderDirectIndexFeed(db, ctx, pageCurPath, security, protoset, scheme, site,
        query, block, pos, blobstores, function(err, result) {
          if (err) {
            return chunk.end();
          }
          chunk.write(result.source);
          return chunk.end();
        });
    });
  };
