 ...
 */
function resultsToIndexFeed(protoset, scheme, response) {
  ...
  response.on('article', function(row) {
    ...
    var article;
    if (protoset) {
      article = protoset.decorateListing(r, scheme);
    } else {
      article = r;
    }
 ...
 */
function emitDirectBlockWithIdx(req, block, pagination, paginationKey, resp, next) {
  var str = '';
  var arts = [];
  var moreLink = '';
  resp.on('article', function(article) {
  ...
  resp.on('end', function() {
    req.scheme.renderSync('partials/list', {site: req.site, data: arts, moreLink: moreLink}, function(err, out) {
      if (err) {
        return next(err);
      }
...
function installDust(dust, db, query) {
  ...
  dust.helpers.basicQuery = function(chunk, context, bodies, params) {
    return chunk.map(function(chunk) {
      ...
      var resp = addIdx(resultsToIndexFeed(protoset, scheme, qr));
...
function renderDirectIndexFeed(req, query, view, block, pos, next) {
  ...
  var idxFeed = addIdx(resultsToIndexFeed(req.protoset, req.scheme, qr));
  var resp = Pagination.generateLastLink(idxFeed, pagination);
  return emitDirectBlockWithIdx(req, block, pagination, paginationKey, resp, next);
}
