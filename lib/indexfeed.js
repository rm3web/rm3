...
function generateFacetLink(row, dates, paginationKey, facetKey, site, sitepath, sort) {
  var date = row.facet;
  if (dates) {
    if (sort === 'changed') {
...
function emitCountWithIdx(chunk, context, bodies, resp, dates, paginationKey, facetKey, site, sitepath, sort) {
  ...
  resp.on('count', function(count) {
    count.idx = idx;
    count.link = generateFacetLink(count, dates, paginationKey, facetKey, site, sitepath, sort);
...
function mergeFacet(queryResponse, facetResponse, facetName, dates, facetKey, site, sitepath, sort, paginationKey) {
  ...
  facetResponse.on('count', function(row) {
    ...
    facet.link = generateFacetLink(row, dates, paginationKey, facetKey, site, sitepath, sort);
    count.push(facet);
  });
  queryResponse.on('error', function(err) {
    ee.emit('error', err);
