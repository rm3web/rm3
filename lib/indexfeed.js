...
function mergeFacet(queryResponse, facetResponse, req, sort) {
  var ee = new events.EventEmitter();
  var queryDone = false;
  var facetDone = false;
  var count = [];
  ...
  facetResponse.on('count', function(row) {
    ...
    if (sort === 'changed') {
      facet.link = req.site.sitePathToUrl(req.sitepath) + '$/changed/' +
        date.getFullYear() + '_' + date.getMonth() + '/';
    } else {
      facet.link = req.site.sitePathToUrl(req.sitepath) + '$/created/' +
        date.getFullYear() + '_' + date.getMonth() + '/';
    }
    count.push(facet);
  });
...
function renderDirectIndexFeed(req, query, view, block, pos, next) {
  ...
  if (facet) {
    resp = mergeFacet(resp, facet, req, block.sort);
  }
