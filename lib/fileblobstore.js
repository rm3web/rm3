...
FileBlobStore.prototype.aliasUnchangedBlob = function(ctx, entityPath, blobPath, revisionId, source, temporary, oldRevisionId, next) {
  var self = this;
  query.findBlob(this._db, ctx, this._config.category, 'fileblobstore', entityPath, blobPath, oldRevisionId, function(err, details) {
    if (err) {
      return next(err);
    }
...
FileBlobStore.prototype.getBlob = function(ctx, entityPath, blobPath, revisionId, next) {
  var self = this;
  query.findBlob(this._db, ctx, this._config.category, 'fileblobstore', entityPath, blobPath, revisionId, function(err, details) {
    if (err) {
      return next(err);
    }
    var filePath = details.filePath;
