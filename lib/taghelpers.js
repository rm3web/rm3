...
function installDust(dust, db, query) {
  ...
  dust.helpers.predTag = function(chunk, context, bodies, params) {
    ...
    if (pred === 'plain') {
      ...
      var predPath = new SitePath(pred);
      chunk.write('<a ' + rel + 'href="/search.cgi/$/tag/' + pred + '">' + predPath.leaf() +
            "</a>");
    }
  };
  ...
  dust.helpers.objLink = function(chunk, context, bodies, params) {
    ...
    if (obj.objClass === 'tag') {
      ...
      chunk.write('<a ' + rel + 'href="/search.cgi/$/ontag/' + predKey + '/' + objKey + '">' +
        objKeyPath.leaf() + "</a>");
    } else if (obj.objClass === 'link') {
      chunk.write('<a href="' + objKey + '">' + xssFilters.inHTMLData(objKey) + '</a> <a ' +
        rel + 'href="/search.cgi/$/link/' + predKey + '/' + encodeURIComponent(objKey) +
        '">(search)</a>');
    } else {
      chunk.write(objKey);
    }
