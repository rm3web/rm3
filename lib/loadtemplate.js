...
var templates = {
  ...
  'ontag': function createOntag(db, Entity, update, item, root, userRoot, next) {
    var entity = new Entity();
    var now = new Date();
    entity.createNew(root.down(item.path), 'ontag', now);
    entity.summary.title = item.title;
    entity.summary.abstract = item.abstract;
    entity.data.properties = item.properties;
    entity.addTag("navigation", "ontag");
    doCreate(db, update, entity, next);
  },
  'page': function createPage(db, Entity, update, item, root, userRoot, next) {
    ...
    entity.data.posting = textblocks.makeTextBlock(item.posting, 'markdown', {site: site});
    entity.summary.title = item.title;
    entity.summary.abstract = item.abstract;
    doCreate(db, update, entity, next);
  },
  'index': function createPage(db, Entity, update, item, root, userRoot, next) {
    ...
    var block = {
      "blocks":[
        textblocks.makeTextBlock(item.posting, 'markdown', {site: site}),
