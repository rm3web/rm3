...
function DeletableMixin(page) {
  page.securityRouter.routeAll('delete.html', authorize({requiresAuth: true, permission: "delete"}));
...
function TagSearchViewMixin(page) {
  page.securityRouter.routeAll('search.cgi', authorize({permission: 'view'}));

  page.viewRouter.routeAll('search.cgi', function(req, res, next) {
...
function CommentableMixin(page) {
  ...
  page.securityRouter.routeAll('comment.cgi', authorize({effectivePermission: 'comment'}));
  page.securityRouter.routeAll('comment.js', authorize({effectivePermission: 'comment'}));

  page.freshnessRouter.routeAll('comment.js', view.noCacheView);

  page.commandRouter.get('comment.js', function(req, res, next) {
    csrfLimiter(req.ipAddress, function(err, timeLeft) {
      if (err) {
  ...
  page.commandRouter.post('comment.cgi', function(req, res, next) {
    ipLimiter(req.ipAddress, function(err, timeLeft) {
      if (err) {
        return res.status(500).send();
      } else if (timeLeft) {
        return res.status(429).send("You must wait " + timeLeft + " ms before you can make requests.");
  ...
  page.commandRouter.post('comment.cgi', function(req, res, next) {
    ...
    userLimiter(req.user.user.path().toDottedPath(), function(err, timeLeft) {
      if (err) {
        return res.status(500).send();
      } else if (timeLeft) {
        return res.status(429).send("You must wait " + timeLeft + " ms before you can make requests.");
