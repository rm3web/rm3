...
function doStandardEdit(oldent, now, req, res, next) {
  ...
  update.updateEntity(req.db, req.ctx, req.access, oldent, req.entity, !saveAsDraft, 'test',
...
function BaseBehaviorMixin(page) {
  ...
  page.commandRouter.post('togglehidden.cgi', authorize({requiresAuth: true, effectivePermission: 'edit'}));
  ...
  page.commandRouter.post('togglenavbar.cgi', function(req, res, next) {
    ...
    update.updateEntity(req.db, req.ctx, req.access, oldent, req.entity, true, 'toggle navbar', function(err) {
      res.status(200);
  ...
  page.commandRouter.post('togglehidden.cgi', function(req, res, next) {
    ...
    update.updateEntity(req.db, req.ctx, req.access, oldent, req.entity, true, 'toggle hidden', function(err) {
      res.status(200);
  ...
  page.commandRouter.put('tags.cgi', function(req, res, next) {
    ...
    if (req.body.hasOwnProperty('tags')) {
      ...
      update.updateEntity(req.db, req.ctx, req.access, oldent, req.entity, true, 'edit tags', function(err, entityId, revisionId, revisionNum) {
        if (err) {
          return next(err);
        }
