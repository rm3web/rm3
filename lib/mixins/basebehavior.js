...
function doStandardEdit(oldent, now, req, res, next) {
  ...
  update.updateEntity(req.db, req.ctx, req.access, oldent, req.entity, !saveAsDraft, sameRevision, 'test',
...
function doStandardEditExpose(req, res, next) {
  var editData = {
    ...
    excludeChildrenDisplay: req.entity.data.excludeChildrenDisplay
  };
...
function doStandardCreateExpose(req, res, next) {
  res.serverVars.bundle = "bundles/page.js";
  res.serverVars.component = "page.jsx";
  next();
}
...
function BaseBehaviorMixin(page) {
  page.securityRouter.routeAll('', authorize({permission: 'view'}));
  page.securityRouter.routeAll('atom.xml', authorize({permission: 'view'}));
  page.securityRouter.routeAll('edit.html', authorize({requiresAuth: true, permission: 'edit'}));
  page.securityRouter.routeAll('publish.html', authorize({requiresAuth: true, permission: 'edit'}));
  page.securityRouter.routeAll('create.html', authorize({requiresAuth: true, effectivePermission: 'edit'}));
  page.securityRouter.routeAll('tag.html', authorize({requiresAuth: true, effectivePermission: 'edit'}));
  ...
  page.freshnessRouter.routeAll('edit.html', view.noCacheView);
  page.freshnessRouter.routeAll('create.html', view.noCacheView);
  ...
  page.viewRouter.routeAll('atom.xml', view.basicView.bind(page, 'atom', 'atom'));
}
