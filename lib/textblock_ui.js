...
exports.generateEditor = function generateEditor(prefix, textblock) {
  if (textblock.format === 'section') {
    ...
    + '[format]" /><input type="hidden" value="' + textblock.blocks.length + '" name="numblocks" />'
    var editors = textblock.blocks.reduce(function(prev, block, index) {
      var blockprefix = prefix + '[blocks][' + index + ']'
      return prev + exports.generateArea(blockprefix + '[source]', selectContentArea(block)) +
        exports.generateSelect(blockprefix + '[format]', block.format);
    },'');
