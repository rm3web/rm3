...
var boundLogger = logging.getRootLogger('db');
/**
 ...
 */
exports.openTransaction = function(ctx, client, done, callback) {
  boundLogger.info('openTransaction', {
    ctx: ctx
  });
 ...
 */
exports.commitTransaction = function(ctx, client, callback) {
  boundLogger.info('commitTransaction', {
    ctx: ctx
  });
 ...
 * @param {Function} callback a function that takes (err)
 */
exports.rollbackTransaction = function(ctx, client, callback) {
  boundLogger.info('rollbackTransaction', {
    ctx: ctx
  });
  ...
  client.query('ROLLBACK', function(err, result) {
    boundLogger.error('commitTransaction failed', {
      ctx: ctx
    });
    if (err) {
...
function handleConnectError(err, queryfunc, done) {
  ...
  if (err.message.match(/^could not connect to server: Connection refused/)) {
    ...
    queryfunc(wrappedError, function() {});
  } else {
 ...
 * @param {Error} err the PG error encountered
 * @return {Error} A wrapped PG error
 */
