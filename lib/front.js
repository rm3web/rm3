...
var sessionTrackerMiddleware = require('express-passport-session-tracker').middleware;
...
phase.addDefaultPhase('session', function(app, next) {
  ...
  app.use(sessionTrackerMiddleware(sessionRedisClient, {}));
  next();
});
...
phase.addDefaultPhase('passport-middleware', function(app, next) {
  ...
  if (forceAuth) {
    ...
    app.use(forceAuthMiddleware(new SitePath(['wh', 'users']), 'wirehead', app.locals.db, query, entity.Entity));
  }
