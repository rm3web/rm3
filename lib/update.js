...
exports.createIdentity = function createIdentity(db, ctx, provider, userId, userPath, providerDetails, next) {
  async.waterfall([
    db.connectWrap,
    function generateIdentity(client, done, callback) {
      var s = squel.insert().into('wh_identity');
      sql.setFields(s, {
        "provider": provider,
        "\"userId\"": userId,
        "\"userPath\"": userPath,
