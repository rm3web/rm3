...
function post_commit_err(db, client, err, done, next) {
  var wrapped_error = db.wrapError(err);
...
function exec_logentry(exec, client, done, logentry, callback) {
  ...
  if (logentry.evt_class === 'create') {
    ...
    sql.set_fields(s, {
      ...
      "tags": JSON.stringify(logentry.data.to_data.tags)
    });
    q = s.toParam();
  ...
  } else if (logentry.evt_class === 'assign') {
  ...
  } else if (logentry.evt_class === 'deassign') {
  ...
  } else if (logentry.evt_class === 'permit') {
  ...
  } else if (logentry.evt_class === 'deny') {
...
/*
  ...
  pred_class
  */
function generate_one_tag_insert(subj_path, pred_class, pred_key, obj_key, obj) {
...
function write_tags(exec, client, done, logentry, callback) {
  ...
  var path = logentry.path;
  var to_data = logentry.data.to_data;
  if (logentry.evt_class === 'move') {
    path = logentry.data.to_path;
    to_data = logentry.data.from_data;
  }
  var queries = [];
  if (to_data) {
    if (to_data.hasOwnProperty('tags')) {
      for (var pred_key in to_data.tags) {
        if (to_data.tags.hasOwnProperty(pred_key)) {
          var pred = to_data.tags[pred_key];
          for (var obj_key in pred) {
            var obj = pred[obj_key];
            var pred_class = obj.pred_class;
            queries.push(
              generate_one_tag_insert(path, pred_class, pred_key, obj_key, obj));
          }
        }
      }
    }
