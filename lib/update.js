...
function log_update(client, path, entity_id, note, base_revision_id, revision_num, 
                    revision_id, evt_start, evt_end, evt_touched, from_data, 
                    to_data, callback) {
  data = {to_data: to_data,
          from_data: from_data};
  insert = client.query({
    text: insert_log_query,
    values: [path, entity_id, note, base_revision_id, null, revision_num, revision_id,
    evt_start, evt_end, evt_touched, 'update', true, data],
    name: 'insert_log_query'
  }, function(err, result){
    callback(err);
  });
...
function log_delete(client, path, entity_id, note, revision_num, revision_id, 
                   evt_start, evt_end, evt_touched, from_data, callback) {
  data = {from_data: from_data};
  insert = client.query({
    text: insert_log_query,
    values: [path, entity_id, note, null, null, revision_num, revision_id,
    evt_start, evt_end, evt_touched, 'delete', true, data],
    name: 'insert_log_query'
  }, function(err, result){
    callback(err);
  });
...
exports.create_entity = function(db, entity, next) {
  ...
  async.waterfall([
    ...
    function log(client, done, result, entity_id, revision_id, revision_num, callback) {
      var to_data = entity.to_log(entity_id, now, now);
...
/**
  ...
  @param {*} next Function that takes(err, entity_id, revision_id, revision_num)
*/
exports.update_entity = function(db, old_entity, new_entity, next) {
  var now = new Date();
  var path = old_entity._path.toDottedPath();
  async.waterfall([
    db.connect_wrap,
    db.open_transaction,
    function update_query_func(client, done, callback){
      var revision_id = uuid.v1();
      var entity_id = old_entity._entity_id;
      var revision_num = old_entity._revision_num + 1;
      insert = client.query({
        text: update_entity_query,
        values: [path, false, entity_id, revision_id, revision_num,
               new_entity._proto, now, now, new_entity.summary, new_entity.data],
    ...
    function log(client, done, result, entity_id, revision_id, revision_num, callback) {
      var from_data = old_entity.to_log(entity_id, now, now);
      var to_data = new_entity.to_log(entity_id, now, now);
      log_update(client, path, entity_id, 'update', null, revision_id, revision_num, 
                   now, now, now, from_data, to_data, function(err) {
        callback(err, client, done, result, entity_id, revision_id, revision_num);
      });
    },
    function commit(client, done, result, entity_id, revision_id, revision_num, callback) {
...
/**
  ...
  @param {*} next Function that takes(err, entity_id, revision_id, revision_num)
*/
exports.delete_entity = function(db, entity, next) {
  var now = new Date();
  var path = entity._path.toDottedPath();
  async.waterfall([
    db.connect_wrap,
    db.open_transaction,
    function delete_query_func(client, done, callback){
      var revision_id = uuid.v1();
      var entity_id = entity._entity_id;
      var revision_num = entity._revision_num + 1;
      client.query({
    ...
    function log(client, done, result, entity_id, revision_id, revision_num, callback) {
      var from_data = entity.to_log(entity_id, now, now);
      log_delete(client, path, entity_id, 'delete', revision_id, revision_num, 
                   now, now, now, from_data, function(err) {
        callback(err, client, done, result, entity_id, revision_id, revision_num);
      });
    },
    function commit(client, done, result, entity_id, revision_id, revision_num, callback) {
