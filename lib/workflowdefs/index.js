    ...
    chain: [{
      ...
      body: function(job, cb) {
        return workflowtasks.renderJpegScale(job.params.ctx, job.params.path, job.params.filename,
          job.params.scalefilename, job.params.revisionId, job.params.sizes, job.params.inBlobstore, job.params.slug, cb);
      }
    }, {
      ...
      body: function(job, cb) {
        return workflowtasks.renderJpegSquare(job.params.ctx, job.params.path, job.params.filename,
          job.params.scalefilename, job.params.revisionId, job.params.sizes, job.params.inBlobstore, cb);
      }
    }],
    timeout: 180,
    onerror: []
