...
function VectorGraphicPage() {
  ...
  this.viewRouter.get('view.jpg', function(req, res, next) {
    ...
    fetchImage(req.ctx, blobstore, req.entity.path().toDottedPath(), 'file-m.jpg', req.entity._revisionId, function(err, path) {
  ...
  this.viewRouter.get('q.jpg', function(req, res, next) {
    var blobstore = req.blobstores.getBlobStore('public');
    fetchImage(req.ctx, blobstore, req.entity.path().toDottedPath(), 'file-s.jpg', req.entity._revisionId, function(err, path) {
      res.redirect(302, path);
    });
  });
  ...
  this.viewRouter.get('sq.jpg', function(req, res, next) {
    var blobstore = req.blobstores.getBlobStore('public');
    fetchImage(req.ctx, blobstore, req.entity.path().toDottedPath(), 'file-sq.jpg', req.entity._revisionId, function(err, path) {
      res.redirect(302, path);
    });
  });
  ...
  this.viewRouter.get('large.jpg', function(req, res, next) {
    var blobstore = req.blobstores.getBlobStore('public');
    fetchImage(req.ctx, blobstore, req.entity.path().toDottedPath(), 'file-l.jpg', req.entity._revisionId, function(err, path) {
      res.redirect(302, path);
    });
  });
  ...
  this.generateFigure = function(ctx, blobstores, entity, options, cb) {
    ...
    var floatStr = '';
    buildSrcSet(ctx, blobstore, entity.path().toDottedPath(), sizes, entity._revisionId, function(err, srcset, exposedThumbnail) {
      var scaleSize;
      if (figSize === 'medium') {
        scaleSize = '33vw';
  ...
  this.viewRouter.get('', function(req, res, next) {
    ...
    buildSrcSet(req.ctx, blobstore, req.entity.path().toDottedPath(), sizes, req.entity._revisionId, function(err, srcset, exposedThumbnail) {
      if (!err) {
