...
function UserPage() {
  ...
  this.commandRouter.post('password.html', function(req, res, next) {
    ...
    passForm.checkForm(req.body, function(err) {
      ...
      user.authenticatePassword(req.db, req.ctx, query, req.entity.data.email,
                                req.body.oldpassword, function(err) {
        if (err) {
          return next(err);
        }
        user.updateCredential(req.db, req.ctx, req.entity.data.email, req.sitepath, req.sitepath.leaf(),
                              req.body.password1, function(err) {
          if (err) {
            return next(err);
          } else {
            return res.redirect(req.site.sitePathToUrl(req.sitepath.down(req.body.path)));
  ...
  this.createFunc = function(now, req, res, next) {
    ...
    update.createEntity(req.db, req.ctx, req.access, req.entity, true,
      ...
      user.createCredential(req.db, req.ctx, req.body.email, req.sitepath, req.body.username,
                            req.body.password1, function(err) {
        if (err) {
          return next(err);
        } else {
  ...
  this.viewRouter.get('', view.basicViewWithJsonView.bind(this, 'user', 'view-user', null));
  this.viewRouter.routeAll('password.html', view.basicView.bind(this, 'password', 'password', null));
