...
function UserPage() {
  ...
  this.securityRouter.get('drafts.cgi', authorize({requiresAuth: true, effectivePermission: 'viewdraft'}));
  this.securityRouter.get('hidden.cgi', authorize({requiresAuth: true, effectivePermission: 'viewdraft'}));
  this.securityRouter.get('moderate.cgi', authorize({requiresAuth: true, effectivePermission: 'viewdraft'}));
  ...
  this.createFunc = function(now, req, res, next) {
    ...
    req.entity.summary.abstract = sanitize.sanitizeXML(req.body.profileText);
  ...
  this.viewRouter.get('moderate.cgi', function(req, res, next) {
    res.format({
      'text/html': function() {
        view.basicView('moderate', 'moderate', req, res, next);
      }
    });
  });
