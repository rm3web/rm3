...
function EmailFormPage() {
  ...
  this.securityRouter.get('email.js', authorize({effectivePermission: 'comment'}));

  this.freshnessRouter.get('email.js', view.noCacheView);

  this.commandRouter.get('email.js', function(req, res, next) {
    req.rateLimiter.csrfEmailLimiter(req.ipAddress, function(err, timeLeft) {
      if (err) {
  ...
  this.securityRouter.post('email.cgi', authorize({effectivePermission: 'comment'}));

  this.freshnessRouter.post('email.cgi', view.noCacheView);

  this.commandRouter.post('email.cgi', function(req, res, next) {
    req.rateLimiter.emailLimiter(req.ipAddress, function(err, timeLeft) {
      if (err) {
        return res.status(500).send();
      } else if (timeLeft) {
        return res.status(429).send("You must wait " + timeLeft + " ms before you can make requests.");
