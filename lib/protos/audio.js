...
function AudioPage() {
  ...
  this.securityRouter.get('listen.mp3', authorize({permission: 'view'}));
  this.viewRouter.get('listen.mp3', function(req, res, next) {
    var blobstore = req.blobstores.getBlobStore('public');
    req.blobstores.fetchBlob(req.ctx, blobstore, req.entity.path().toDottedPath(), 'file.mp3', req.entity._revisionId, function(err, path) {
