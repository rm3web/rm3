...
function fetchBothImages(ctx, blobstore, path, filename, svgname, sizes, scale, revisionId, cb) {
  fetchImage(ctx, blobstore, path, filename, revisionId, function(err, blobpath) {
    ...
    fetchImage(ctx, blobstore, path, svgname, revisionId, function(err, svgpath) {
      if (blobpath && svgpath) {
        ...
        var str = '<picture><source srcset="' + svgpath +
...
function VectorGraphicPage() {
  ...
  this.generateFigure = function(ctx, blobstores, entity, options, cb) {
    ...
    if (figSize === 'small') {
      fetchBothImages(ctx, blobstore, entity.path().toDottedPath(), 'file-s.png', 'file.svg', sizes, 240, entity._revisionId, cb);
    } else if (figSize === 'medium') {
      fetchBothImages(ctx, blobstore, entity.path().toDottedPath(), 'file-m.png', 'file.svg', sizes, 500, entity._revisionId, cb);
    } else if (figSize === 'large') {
      fetchBothImages(ctx, blobstore, entity.path().toDottedPath(), 'file-l.png', 'file.svg', sizes, 1024, entity._revisionId, cb);
    } else {
