...
function fetchBothImages(ctx, blobstores, blobstore, path, filename, svgname, sizes, scale, revisionId, floatStr, cb) {
  blobstores.fetchBlob(ctx, blobstore, path, filename, revisionId, function(err, blobpath) {
    if (err) {
      return cb(err, svgPlaceholderBox);
    }
    blobstores.fetchBlob(ctx, blobstore, path, svgname, revisionId, function(err, svgpath) {
      if (blobpath && svgpath) {
...
function VectorGraphicPage() {
  ...
  this.viewRouter.get('download.cgi', function(req, res, next) {
    ...
    req.blobstores.fetchBlob(req.ctx, blobstore, req.entity.path().toDottedPath(), 'file.png', req.entity._revisionId, function(err, path) {
