...
function doPragmaBlocks(req, view, block, next) {
  ...
  resp.on('article', function(article) {
    var data = req.protoset.decorateListing(article);
    data.$idx = idx;
    arts.push(data);
    idx = idx + 1;
  });
  ...
  resp.on('end', function() {
    async.reduce(arts, str, function(memo, data, callback) {
      ...
      return next(err, {format: 'html', htmltext: result + '</ul>'});
    });
  });
}
...
Protoset.prototype.decorateListing = function(article) {
  var data = {path: article.path.toUrl('/', 1),
              article: article};
  var tag = article['rm3:proto'];
  if (this._metadata.hasOwnProperty(tag) && this._metadata[tag].hasOwnProperty('icon')) {
    data.icon = {'svg': '/resources/images/icon-' + this._metadata[tag].icon + '.svg',
      'sq': '/resources/images/icon-' + this._metadata[tag].icon + '-75.png'
    };
  } else {
    data.icon = {'svg': '/resources/images/icon-document.svg',
      'sq': '/resources/images/icon-document-75.png'
    };
  }
  return data;
};
