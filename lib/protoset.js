...
function HistoryViewMixin(page) {
  page.viewRouter.get('history.cgi', function(req, res, next) {
    res.format({
      'application/stream+json': function() {
        ActivityFeed.renderDirectJsonPayload(query, req.db, req.ctx, req.security, req.sitepath, 
          req.site, req.revisionId, function(err, data){
            if (err) {
