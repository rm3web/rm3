...
function UserPage() {
  ...
  this.commandRouter.routeAll('edit.html', function(req, res, next) {
    view = req.entity.view();
    res.expose(req.entity.path().leaf(), 'username');
    res.expose(view.summary.title, 'fullname');
    res.expose(view.data.email, 'email');
    res.expose(view.summary.abstract, 'profileText');
    res.expose(view.data.profileUrl, 'profileUrl');
    next();
  });
