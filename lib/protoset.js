...
function CommentPage() {
  ...
  this.viewRouter.get('', basicView.bind(this, undefined, 'view-comment', null));
}
...
function BlogPage() {
  ...
  this.viewRouter.get('', csrfProtection);

  this.viewRouter.get('', function(req, res, next) {
    res.expose(req.csrfToken(), 'csrfToken');
    res.expose(req.site.sitePathToUrl(req.sitepath) + 'comment.cgi', 'commentPath');
    next();
  });

  this.viewRouter.get('', basicView.bind(this, undefined, 'view-blog', null));
  ...
  this.commandRouter.post('comment.cgi', function(req, res, next) {
    res.format({
      'application/json': function() {
        if (req.entity.permissions.hasOwnProperty('comment')) {
          var commentPath = uuid.v1().replace(/-/g,'_');
          var now = new Date();
          var comment = new req.Entity();
          comment.createNew(req.sitepath.down(commentPath), 'comment', now);
          comment.data.comment = req.body.comment;
          update.createEntity(req.db, req.ctx, req.access, comment, true, 'comment',
            function(err, entityId, revisionId, revisionNum) {
              if (err) {
                return next(err);
              } else {
        ...
        } else if (req.entity.permissions.hasOwnProperty('submitcomment')) {
          res.status(202);
...
Protoset.prototype.decorateListing = function(article, scheme, site) {
