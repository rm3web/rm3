...
function checkFieldsPresent(ctx, body, fields, next) {
  ...
  if (badFields.length > 0) {
    logging.logAndWrapError(boundLogger, 'checkFieldsPresent', 
...
function authorize(options) {
  return function auth(req, res, next) {
    ...
    if (options.requiresAuth &&
        !user) {
      return logging.logAndWrapError(boundLogger, 'authorize not allowed', 
        'page.not_allowed', {
        ctx: req.ctx,
    ...
    if (options.permission &&
      ...
      return logging.logAndWrapError(boundLogger, 'authorize no permission', 
        'page.not_allowed', {
        ctx: req.ctx,
    ...
    if (options.effectivePermission) {
      query.fetchEffectivePermissions(req.db, user.path(), req.sitepath, function(err, effPerm) {
        ...
        if (!effPerm.hasOwnProperty(options.effectivePermission)) {
          return logging.logAndWrapError(boundLogger, 'authorize no effective permission', 
            'page.not_allowed', {
            ctx: req.ctx,
            reason: "Your user does not have the required permission"
          }, next);
...
function BaseBehaviorMixin(page) {
  ...
  page.commandRouter.post('tag.html', function(req, res, next) {
    ...
    if (req.body.action === "addtag") {
      ...
      return logging.logAndWrapError(boundLogger, 'tag.html bad action', 
            'page.validation', {
            ctx: req.ctx,
  ...
  page.commandRouter.post('create.html', function(req, res, next) {
    ...
    if (validator.toBoolean(req.body.autogenSlug)) {
      ...
      if (validator.isNull(req.body.path)) {
        return logging.logAndWrapError(boundLogger, 'create.html bad path', 
            'page.validation.empty', {
            ctx: req.ctx,
            field: 'path'
          }, next);
...
function UserPage() {
  ...
  this.commandRouter.post('edit.html', function(req, res, next) {
    ...
    if (checkFieldsPresent(req.ctx, req.body, ['fullname'], next)) {
      return logging.logAndWrapError(boundLogger, 'edit.html bad field',
          'page.validation', {
          ctx: req.ctx,
    ...
    if (req.body.password1 !== req.body.password2) {
      return logging.logAndWrapError(boundLogger, 'edit.html pw1 pw2',
          'page.validation', {
          ctx: req.ctx,
    ...
    if (!comboCheck(req.body.profileUrl, req.entity.summary, 'profileUrl',
      ...
      return logging.logAndWrapError(boundLogger, 'edit.html profileUrl',
          'page.validation', {
          ctx: req.ctx,
          msg: "bad profile url",
          field: ['profileUrl']
    ...
    if (!comboCheck(req.body.email, req.entity.data, 'email',
      ...
      return logging.logAndWrapError(boundLogger, 'edit.html email',
          'page.validation', {
          ctx: req.ctx,
          msg: "bad email",
          field: ['email']
  ...
  this.commandRouter.post('create.html', function(req, res, next) {
    ...
    if (checkFieldsPresent(req.ctx, req.body, ['username', 'fullname'], next)) {
      return logging.logAndWrapError(boundLogger, 'create.html bad field',
          'page.validation', {
          ctx: req.ctx,
          msg: "required fields missing",
    ...
    if (req.body.password1 !== req.body.password2) {
      return logging.logAndWrapError(boundLogger, 'create.html pw1 pw2',
          'page.validation', {
          ctx: req.ctx,
          msg: "passwords don't match",
          field: ['password1', 'password2']
    ...
    if (!comboCheck(req.body.profileUrl, req.entity.summary, 'profileUrl',
      ...
      return logging.logAndWrapError(boundLogger, 'create.html profileUrl',
          'page.validation', {
          ctx: req.ctx,
          msg: "bad profile url",
          field: ['profileUrl']
    ...
    if (!comboCheck(req.body.email, req.entity.data, 'email',
      ...
      return logging.logAndWrapError(boundLogger, 'create.html email',
          'page.validation', {
          ctx: req.ctx,
          msg: "bad email",
          field: ['email']
