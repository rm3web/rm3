...
var update = require ('./update'),
    ...
    UserForm = require('./forms/user').UserForm;
...
function UserPage() {
  ...
  this.commandRouter.post('edit.html', function(req, res, next) {
    ...
    var f = new UserForm({data: data});

    if (f.isValid()) {
      encodePasswordOrNot(req.body.password, req.entity,
        !req.body.password, function(err) {
          ...
          req.entity.data.email = data.email;
          req.entity.summary.profileUrl = data.profileUrl;
          req.entity.summary.title = data.fullname;
          req.entity.summary.abstract = data.profileText;
          update.updateEntity(req.db, req.ctx, oldent, req.entity, true, 'test', next);
        });
    } else {
      return logging.logAndCreateError(boundLogger, f.errors().asText(),
          'page.validation', {
          ctx: req.ctx,
          msg: f.errors().asText(),
          field: []
  ...
  this.commandRouter.post('create.html', function(req, res, next) {
    ...
    if (f.isValid()) {
      encodePasswordOrNot(req.body.password, req.entity, !req.body.password, function(err) {
        if (err) {
          return next(err);
        }
