...
function UserPage() {
  ...
  this.command_router.addRoute('/POST/create', function(req, res, page, db, next)
  {
    ...
    req.entity.createNew(req.sitepath.down(req.body.username), page.proto, now);
    req.entity.summary.abstract = req.body.abstract;
    ...
    if (!validator.isNull(req.body.profile_url)) {
      if (!validator.isURL(req.body.profile_url)) {
        return next(errs.create('page.validation', {
          field: 'profile_url'
        }));
      } else {
        req.entity.summary.profile_url = req.body.profile_url;
      }
    }

    
    if (!validator.isNull(req.body.email)) {
      if (!validator.isURL(req.body.email)) {
        return next(errs.create('page.validation', {
          field: 'email'
        }));
      } else {
        req.entity.data.email = req.body.email;
      }
    }
    ...
    crypto.randomBytes(Conf.get_security('pbkdf2_saltlen'), function(err, buf) {
      ...
      crypto.pbkdf2(req.body.password1, salt, Conf.get_security('pbkdf2_iterations'), 
                    Conf.get_security('pbkdf2_keylen'), function(err, hashRaw) {
        if (err) {
          return next(err);
        }
        req.entity.data.hash = new Buffer(hashRaw, 'binary').toString('hex');
        req.entity.data.salt = salt;
