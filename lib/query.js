...
exports._read_logentry = function(revisionId, client, done, callback) {
  var s = squel.select().from('wh_log');
  sql.search_fields(s,sql.log_fields()).where('"revisionId" = ?', revisionId);
  var q = s.toParam();
...
/**
  ...
  @param {*} next Function that takes(err, entity)
*/
exports.entity_from_entityId = function(db, Entity, access, ntity_id, revision_id, next) {
...
exports.query_history = function(db, access, path) {
  var s = squel.select().from('wh_log');
  sql.search_fields(s,sql.log_fields()).where('path = ?', path.toDottedPath()).order('"revisionNum"');
  var q = s.toParam();
