...
exports._activityQueryGen = function(access, curpath, select, userpath) {
  var doJoin = true;
...
exports.queryActivity = function(db, ctx, access, curpath, select, userpath) {
  ...
  var q = exports._activityQueryGen(access, curpath.toDottedPath(), select, userpath);
  ...
  return recordQuery(db, ctx, q, function(ee, row) {
    var r = {
      ...
      actorPath: row.actorPath,
      data: row.data,
      objProto: row.objProto,
      objSummary: row.objSummary,
      actorProto: row.actorProto,
      actorSummary: row.actorSummary
    };
    r.path = new SitePath();
    r.path.fromDottedPath(row.path);
    if (r.actorPath !== 'root') {
      r.actorPath = new SitePath();
      r.actorPath.fromDottedPath(row.actorPath);
    }
    ee.emit('article', r);
  });
};
