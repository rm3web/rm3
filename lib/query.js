...
function IdentityNotFoundError() {
  this.message = "Identity not found";
}
util.inherits(IdentityNotFoundError, Error);
errs.register('query.identity_not_found', IdentityNotFoundError);
 ...
 * @param {string} select `child`, `dir`, or `parents`.
 * @param {SitePath|null} userpath The user's path we are looking at
 * @param {Object} filter The filtersg
...
/**
 */
exports.findIdentity = function(db, ctx, provider, userId, next) {
  var q;
  boundLogger.info('findIdentity', {
    ctx: ctx,
    provider: provider,
    userId: userId
  });
  async.waterfall([
    db.connectWrap,
    function(client, done, callback) {
      var s = squel.select().from('wh_identity')
       .field('provider')
       .field('"userId"')
       .field('"userPath"')
    ...
    if (err) {
      return logging.logAndWrapError(boundLogger, err,
        'findIdentity error', 'query.error', {
          ctx: ctx,
          selectEntityQuery: q
        }, next);
    }
    if (result.rowCount === 0) {
      return logging.logAndCreateError(boundLogger,
        'findIdentity rowCount',
        'query.identity_not_found', {
          ctx: ctx,
        }, next);
    }
    var row = result.rows[0];
    var user = new SitePath();
    user.fromDottedPath(row.userPath);
    row.userPath = user;
    next(err, row);
  });
};
