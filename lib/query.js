 ...
 * @param {Function} next a function that returns (err, touched, revision_id, revision_nu)
 */
exports.fetchMostRecentChange = function(db, ctx, next) {
  var q;
  ...
  async.waterfall([
    ...
    next(err, row.modified, row.revisionId, row.revisionNum);
  });
};
 ...
 */
exports.findBlob = function(db, ctx, category, provider, entityPath, blobPath, revisionId, next) {
  var q;
