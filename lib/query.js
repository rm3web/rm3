...
exports._queryGen = function(access, curpath, select, target, filter, sort, facet, pagination) {
  ...
  if (filter.hasOwnProperty('ontags')) {
    tagJoin = true;
    s.where(
      squel.expr()
        .and("\"predPath\" = 'navigation'")
  ...
  if (filter.hasOwnProperty('tag')) {
    ...
    if (target !== 'count') {
      s.order("wh_entity.path");
      s.distinct("wh_entity.path");
    }
  }
  ...
  if (facet && facet.hasOwnProperty('objClass')) {
    s.where('"objClass" = ?', facet.objClass);
  }
...
function _fetchMostRecentChange(db, ctx, path, next) {
  var q;
 ...
 * @param {SitePath} path The path
 * @param {Function} next a function that returns (err, touched, revision_id, revision_num)
 */
exports.fetchMostRecentChange = function(db, cache, ctx, path, next) {
  cache.get('mostRecentChange:' + path.path[0], function(err, response) {
    if (err) {
      return next(err);
    }
    if (response) {
      ...
      _fetchMostRecentChange(db, ctx, path, function(err, touched, revisionId, revisionNum) {
        if (err) {
          return next(err);
        }
        var value = {touched: touched, revisionId: revisionId, revisionNum: revisionNum};
        cache.set('mostRecentChange:' + path.path[0], value, 60);
 ...
 * @param {Object} ctx The logging context
 * @param {string} provider The provider (e.g. "twitter")
 * @param {string} clientId The user ID in the provider's space
 * @param {Function} next a function that returns (err, identity)
 */
exports.findServiceAccount = function(db, ctx, provider, clientId, next) {
  var q;
  boundLogger.info('findServiceAccount', {
    ctx: ctx,
    provider: provider,
    clientId: clientId
  });
  async.waterfall([
    db.connectWrap,
    function(client, done, callback) {
      var s = squel.select().from('wh_serviceaccount')
       .field('provider')
       .field('"clientId"')
       .field('"providerDetails"')
       .where('provider = ?', provider)
    ...
    if (err) {
      return logging.logAndWrapError(boundLogger, err,
        'findServiceAccount error', 'query.error', {
          ctx: ctx,
          selectEntityQuery: q
        }, next);
    }
 ...
 */
function _findBlob(db, ctx, category, provider, entityPath, blobPath, revisionId, next) {
  var q;
...
exports.findBlob = function(db, cache, ctx, category, provider, entityPath, blobPath, revisionId, next) {
  var cacheKey = 'b:' + category + '-' + provider + '-' + entityPath + '-' + revisionId + '-' + blobPath;
  if (!cache) {
    return _findBlob(db, ctx, category, provider, entityPath, blobPath, revisionId, next);
  }
  cache.get(cacheKey, function(err, response) {
    if (err) {
      return next(err);
    ...
    if (response) {
      boundLogger.info('findBlob cache hit', {
        ctx: ctx,
        provider: provider,
        entityPath: entityPath,
        blobPath: blobPath,
      ...
      _findBlob(db, ctx, category, provider, entityPath, blobPath, revisionId, function(err, response) {
        if (err) {
          if (err.name === 'BlobNotFoundError') {
            cache.set(cacheKey, {notFound: true}, 10);
            return next(err, response);
          }
          return next(err);
        }
        cache.set(cacheKey, {response: response}, 1200);
        next(err, response);
      });
    }
  });
