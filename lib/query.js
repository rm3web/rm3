...
function fetch_via_entity_table(db, Entity, access, path, next) {
  var q;
  async.waterfall([
    db.connect_wrap,
...
function fetch_via_log(db, Entity, access, path, revision_id, next) {
  async.waterfall([
    db.connect_wrap,
...
exports.entity_from_path = function(db, Entity, access, path, revision_id, next) {
  if (revision_id) {
    return fetch_via_log(db, Entity, access, path, revision_id, next);
  } else {
    return fetch_via_entity_table(db, Entity, access, path, next);
  }
};
