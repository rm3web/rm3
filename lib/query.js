...
var select_entity_query = "SELECT path, stub, entity_id, revision_id, revision_num, \
proto, modified, created, summary, data FROM wh_entity WHERE path = $1;";
var select_log_query = "SELECT path, entity_id, note, base_revision_id, \
...
var select_log_path_query = "SELECT path, entity_id, note, base_revision_id, \
replace_revision_id, revision_id, revision_num, evt_start, evt_end, \
evt_touched, evt_class, evt_final, data FROM wh_log WHERE path = $1 ORDER BY revision_num;";
...
exports._query_gen = function(curpath, select, target, filter, sort, facet) {
  var s = squel.select()
    .from('wh_entity');
  if (target === 'entity') {
    sql.search_fields(s,sql.entity_fields);
...
exports.query_history = function(db, path) {
  var ee = new events.EventEmitter();
  async.waterfall([
    db.connect_wrap,
    function(client, done, callback) {
      var qr = client.query({
        text: select_log_path_query,
        values: [path.toDottedPath()],
        name: 'select_log_path_query'
      });
...
exports.query = function(db, curpath, select, target, filter, sort, facet) {
  var ee = new events.EventEmitter();
  async.waterfall([
    db.connect_wrap,
    function(client, done, callback){
      var q = exports._query_gen(curpath.toDottedPath(), select, target, filter, sort, facet);
      ...
      qr.on('row', function(row) {
        if (target === 'entity') {
          ...
          callback(errs.create('query.invalid', {
            details: 'target not a valid type'
          }));
        }
      });
