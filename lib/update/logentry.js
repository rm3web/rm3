...
exports.execLogentry = function execLogentry(ctx, exec, client, done, logentry, callback) {
  ...
  if (logentry.evtClass === 'Create') {
    ...
    s = squel.update().table('wh_entity');
    var summary = {'deleted': true};
    if (logentry.data.redirect) {
      summary.redirect = logentry.data.redirect;
    }
    sql.setFields(s, {
      "stub": true,
      "\"entityId\"": logentry.entityId,
    ...
    client.query(q, function(err, result) {
      ...
      sql.setFields(s, {
        "path": logentry.path,
        "stub": true,
        "\"entityId\"": logentry.data.stubEntityId,
        "\"revisionId\"": logentry.data.stubRevisionId,
        "\"revisionNum\"": logentry.revisionNum,
        "proto": null,
        "modified": logentry.evtTouched,
        "created": logentry.data.fromData.created,
      ...
      q = s.toParam();
      q.name = 'delete_entity_query_stub';
      client.query(q, function(err, result) {
        return callback(err, client, done, logentry);
      });
