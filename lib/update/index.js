...
exports.expungeEntity = function deleteBlob(db, ctx, path, next) {
  ...
  async.waterfall([
    ...
    function deleteGeotags(client, done, callback) {
      var s = squel.remove().from('wh_geotag');
      s.where("\"subjPath\" = ?", entityPath);
...
/**
  Adds a ticket (represents a vote, a abuse report, et aL)
  @param {Object} db DB wrapper
  @param {Object} ctx Logging context
  @param {SitePath} entityPath The path associated with this ticket
  @param {String} inetAddr The address associated with this ticket
  @param {String|null} identityId The identity associated with this ticket
  @param {SitePath|null} userPath The user associated with this ticket
  @param {Date} recorded The time this ticket was recorded
  @param {String} subject The address associated with this ticket
  @param {Object} details The details associated with this ticket
  @param {*} next Function that takes(err, ticketId)
*/
exports.addTicket = function addBlob(db, ctx, entityPath, inetAddr, identityId, userPath, recorded,
                                   subject, details, next) {
  var ticketId = uuid.v1();
  async.waterfall([
    db.connectWrap,
    function insertTicket(client, done, callback) {
      var uPath;
      if (userPath) {
        uPath = userPath.toDottedPath();
      }
      var s = squel.insert().into('wh_ticket');
      sql.setFields(s, {
        "path": entityPath.toDottedPath(),
        "\"inetAddr\"": inetAddr,
      ...
      var q = s.toParam();
      q.name = 'add_ticket_query';
      return client.query(q, function(err) {
        callback(err, client, done, {});
      });
    }
  ], doCleanup.bind(this, db, ctx, function(err) {
    if (err) {
      return next(err);
    }
    next(err, ticketId);
