...
function authenticatePassword(password, entity, next) {
  var self = this;
  crypto.pbkdf2(password, entity.data.secrets.salt, Conf.get_security('pbkdf2_iterations'), 
                Conf.get_security('pbkdf2_keylen'), function(err, hashRaw) {
    if (err) {
        return next(err);
    }
...
function findByUsername(db, query, entity, userpath, username, callback) {
  ...
  query.entity_from_path(db, entity, security, userpath.down(username), null, callback);
}
