...
function loadEntity(db, security, parsedData, entityPath, preserveEntityId, loadHistory, revisions, next) {
...
function loadEntities(db, security, catalog, catalogPath, loadHistory, next) {
  var conf = {
  ...
  async.forEachOf(catalog, function(value, key, callback) {
    if (!key.startsWith('_')) {
      ...
      async.waterfall([
        ...
        loadEntity.bind(this, db, security, jsonfile.readFileSync(inPath), null, true, loadHistory)
      ], callback);
    } else {
      callback();
    }
...
function loadFromCatalog(db, security, catalogPath, entityPath, userPath, loadHistory, next) {
  var catalog = jsonfile.readFileSync(path.join(catalogPath, 'catalog.json'));
  async.series([
    loadEntities.bind(this, db, security, catalog, catalogPath, loadHistory),
    function(callback) {
