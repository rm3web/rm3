...
function loadEntityWithHistory(db, security, ent, history, preserveEntityId, next) {
  if (history) {
    ...
    async.forEachOfSeries(history, function(entry, key, callback) {
      ...
      // TODO: This really ought to be a list of the revisions
      next(err, ent._entityId, null, null);
    });
  } else {
    update.loadEntity(db, {}, security, ent, true,
      'loaded via rm3load', preserveEntityId, function(err, entityId, revisionId, revisionNum) {
        ...
        next(err, entityId, revisionId, revisionNum);
      });
  }
}

function loadEntity(db, security, site, parsedData, entityPath, preserveEntityId, loadHistory, revisions, next) {
  ...
  loadEntityWithHistory(db, security, ent, history, preserveEntityId,
    function(err, entityId, revisionId, revisionNum) {
      ...
      if (page.postCreate) {
        ...
        async.forEachOf(revisions, function(key, value, callback) {
          page.postCreate({}, site, ent, value, callback);
        }, next);
      } else {
        next(undefined);
      }
...
function loadEntities(db, security, site, catalog, catalogPath, loadHistory, next) {
  var conf = {
    path: Conf.getPath('localBlobs'),
    urlroot: '/blobs/',
    category: 'public'
  };
  var store = BlobStores.register('public', new FileBlobStore(conf, db, null));
  ...
  async.forEachOf(catalog, function(value, key, callback) {
    if (!key.startsWith('_')) {
      ...
      async.waterfall([
        ...
        loadEntity.bind(this, db, security, site, jsonfile.readFileSync(inPath), null, true, loadHistory)
      ], callback);
    } else {
      callback();
    }
...
function loadFromCatalog(db, security, site, catalogPath, entityPath, userPath, loadHistory, next) {
  var catalog = jsonfile.readFileSync(path.join(catalogPath, 'catalog.json'));
  async.series([
    loadEntities.bind(this, db, security, site, catalog, catalogPath, loadHistory),
    function(callback) {
      ...
      if (catalog._permissions) {
        ...
        loadTemplate(db, entity.Entity, update,
          jsonfile.readFileSync(permissionsPath), entityPath, userPath, {'assign': true}, callback);
      } else {
        callback();
      }
    },
