...
function loadEntity(db, parsedData, entityPath, revisions, next) {
...
function loadBlobs(catalog, store, catalogPath, next) {
  ...
  async.forEachOf(catalog.blobs, function(value, key, callback) {
    ...
    fs.readFile(filepath, function(err, data) {
      ...
      store.addBlob({}, value.entityPath, value.blobPath, value.revisionId, value.source,
  ...
  async.forEachOf(catalog, function(value, key, callback) {
    if (!key.startsWith('_')) {
      ...
      async.waterfall([
        ...
        loadEntity.bind(this, db, jsonfile.readFileSync(inPath), null)
      ], callback);
    } else {
      callback();
    }
