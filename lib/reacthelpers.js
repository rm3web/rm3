...
var renderComponentToString = function renderComponent(reactDirs, filepath, props) {
  var component;
  for (var i = reactDirs.length - 1; (i >= 0 && component === undefined); --i) {
    if (fs.existsSync(path.join(reactDirs[i], filepath))) {
      component = requireCompiled(path.resolve(path.join(reactDirs[i], filepath)));
    }
  }
