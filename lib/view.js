  ...
  */
  ...
  setTimeout(function() {
    IndexFeed.renderDirectIndexFeed(ctx.db, ctx.ctx, ctx.sitepath, ctx.access,
      ctx.protoset, ctx.scheme, ctx.site, query, input, pos, function(err, text) {
...
function doPragmaBlocks(req, view, block, pos, next) {
  return IndexFeed.renderDirectIndexFeed(req.db, req.ctx, req.sitepath, req.access, req.protoset, req.scheme, req.site, query, block, pos, next);
}
