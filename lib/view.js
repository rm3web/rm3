...
textblocks.registerEnhancement('siteImage',function enhanceHtml($, generatePlaceholder, ctx) {
  $('img').each(function(i, elem) {
    ...
    if (!parsedUrl.host) {
      var sitepath = Sitepath.fromUrlSegment(parsedUrl.pathname, ctx.site.root.pathArray());
      var attr = $(this).attr('');
  ...
  query.entityFromPath(ctx.db, ctx.cache, entity.Entity, ctx.ctx, ctx.access, sitepath, null, function(err, newentity) {
    ...
    return ctx.protoset.generateFigure(ctx.ctx, ctx.blobstores, ctx.site, newentity, input, callback);
  });
});
...
function basicViewSetup(viewIn, req, res) {
...
function basicView(section, template, req, res, next) {
  var view = basicViewSetup(req.entity.view(), req, res);
...
exports.basicViewSetup = basicViewSetup;
