...
exports.passportPaths = function(app) {
  ...
  if (!Conf.getConfig('disableHttpsChecks')) {
    app.get('/\\$login/', requireHttps(403, 'HTTPS is required to log in.'));
    app.get('/\\$login/password', requireHttps(403, 'HTTPS is required to log in.'));
  }

  app.get('/\\$login/', csrfProtection, function(req, res) {
    res.cacheControl.noCache();
    var data = view.basicViewSetup(req, res);
  ...
  app.post('/\\$login/password', csrfProtection, function(req, res, next) {
