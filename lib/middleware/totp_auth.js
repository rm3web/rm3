...
exports.passportPaths = function(db, app) {
  ...
  app.get('/\\$login/totp', csrfProtection, function(req, res, next) {
    if (!req.user) {
      res.redirect('/$login');
    }
  ...
  app.post('/\\$login/totp/enroll', csrfProtection, function(req, res, next) {
    user.authenticatePassword(db, req.ctx, query, req.user.user.data.email, req.body.password, function(err) {
      if (err) {
      ...
      if (notp.totp.verify(req.body.token, base32.decode(secret))) {
        update.createCredential(db, req.ctx, 'totp', req.user.user.data.email, req.user.user.path().toDottedPath(), {secret: req.body.secret}, function(err) {
          if (err) {
            console.log(err);
            return res.redirect('/$login/totp/enroll');
          }
