...
exports.passportPaths = function(db, app) {
  ...
  if (!Conf.getConfig('disableHttpsChecks')) {
    app.post('/\\$login/twitter', requireHttps(403, 'HTTPS is required to log in.'));
    app.get('/\\$login/twitter', requireHttps(403, 'HTTPS is required to log in.'));
    app.get('/\\$login/twitter/connect', requireHttps(403, 'HTTPS is required to log in.'));
    app.get('/\\$login/twitter/callback', requireHttps(403, 'HTTPS is required to log in.'));
    app.get('/\\$login/twitter/authorize', requireHttps(403, 'HTTPS is required to log in.'));
  }
