...
describe('query gen', function() {
  ...
  describe('generates the correct queries', function() {
    var tests = [
      ...
      {desc: 'for full text search',
        args: [root, 'wh', 'child', 'count', {fullText: "blah"}, undefined, undefined, {}],
        expected: 'SELECT count(*) FROM wh_entity WHERE (wh_entity.stub <> true) AND (wh_entity.path <@ $1) AND (wh_entity.hidden = false) AND (wh_entity.search @@ to_tsquery($2))'},
      ...
      {desc: 'for sorting by random',
        args: [root, 'wh', 'child', 'entity', {}, 'random', undefined, {}],
        expected: 'SELECT path, stub, hidden, "entityId", "revisionId", "revisionNum", proto, modified, created, touched, summary, data, tags FROM wh_entity WHERE (wh_entity.stub <> true) AND (wh_entity.path <@ $1) AND (wh_entity.hidden = false) ORDER BY random() ASC'},
