  ...
  describe('generates the correct queries', function() {
    var tests = [
      ...
      {desc: 'for faceting by month created',
       ...
       expected: 'SELECT count(*), date_trunc(\'month\', created) AS facet FROM wh_entity WHERE (wh_entity.stub <> true) AND (wh_entity.path <@ $1) GROUP BY date_trunc(\'month\', created) ORDER BY date_trunc(\'month\', created) DESC'},
      {desc: 'for faceting by month modified',
       args: [root, 'wh', 'child', 'count', {}, 'changed', {on: 'month'}, {}],
       expected: 'SELECT count(*), date_trunc(\'month\', modified) AS facet FROM wh_entity WHERE (wh_entity.stub <> true) AND (wh_entity.path <@ $1) GROUP BY date_trunc(\'month\', modified) ORDER BY date_trunc(\'month\', modified) DESC'},
