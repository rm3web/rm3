  ...
  describe('generates the correct queries', function() {
    var tests = [
      ...
      {desc: 'for plain tags',
        ...
        expected: 'SELECT DISTINCT ON ("objStr", "predPath") count(*) FROM wh_entity INNER JOIN wh_tag ON (wh_tag."subjPath" = wh_entity.path) WHERE (wh_entity.stub <> true) AND (wh_entity.path <@ $1) AND (wh_entity.hidden = false) AND ("objStr" = $2) AND ("predPath" = $3) ORDER BY "objStr" ASC, "predPath" ASC'},
      {desc: 'for predicates',
